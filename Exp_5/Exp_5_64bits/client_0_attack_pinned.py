# ============================================================
# client_0_attack_pinned.py : CLIENT 0 — EXPÉRIENCE 3 (attaques + blockchain)
# ============================================================
# Basé sur client_0_pinned.py (Expérience 1, épinglage cœur 1 conservé
...
#    puisse tracer le bon client dans le registre blockchain (avant,
#    le serveur utilisait client_proxy.cid, un UUID Flower interne
#    non fiable pour identifier PRÉCISÉMENT qui a fait quoi)
#
# LANCER :
#   python client_0_attack_pinned.py --server_ip 127.0.0.1
#   python client_0_attack_pinned.py --server_ip 127.0.0.1 --attack_type label_flip
#   python client_0_attack_pinned.py --server_ip 127.0.0.1 --attack_type backdoor
# ============================================================
...
# ne baisse presque pas, contrairement au label-flipping (attaque
# furtive vs attaque visible : c'est le contraste qu'on veut montrer
# dans l'article).
...
def apply_trigger(images, trigger_size=3, trigger_value=1.0):
    ...
    # .clone() : méthode PYTORCH — copie le tenseur pour ne pas
    # modifier les images originales en mémoire
    images_triggered = images.clone()

    # on écrase les derniers pixels (bas-droite) avec trigger_value
...
def train_backdoor(model, train_loader, epochs=1, poison_fraction=0.5, target_label=0, trigger_size=3):
    ...
    normal → le client garde une bonne accuracy globale, ce qui rend
    cette attaque discrète.
...
def test_backdoor_success(model, test_loader, target_label=0, trigger_size=3):
    ...
    le trigger ajouté, quelle proportion le modèle classe-t-il en
    target_label au lieu du vrai chiffre ? C'est la métrique clé du
    backdoor, en plus de l'accuracy normale (qui elle reste stable)."""
...
class MNISTClient(fl.client.NumPyClient):
    ...
    def __init__(self, client_id, attack_type="none"):
        ...
        # remplace l'ancien flag booléen malicious=True/False — NOUVEAU
        # (Expérience 1 ne connaissait que 2 états, Expérience 3 en a 3)
    ...
    def fit(self, parameters, config):
        ...
        # 1. on charge les poids globaux envoyés par le serveur
        # 2. on entraîne localement (honnête, label-flip ou backdoor)
        # 3. on retourne les poids mis à jour
        ...
        # NOUVEAU (Expérience 3) : on renvoie client_id + attack_type dans
        # les métriques, au lieu d'un dictionnaire vide {}. C'est CE qui
        # permet au serveur de tracer le bon client dans le registre
        # blockchain (voir server_attack_blockchain_pinned.py), plutôt
        # que de dépendre du cid Flower interne (UUID non fiable)
        metrics = {
    ...
    def evaluate(self, parameters, config):
        ...
        # NOUVEAU (Expérience 3) : si ce client fait du backdoor, on
        # calcule aussi l'ASR (Attack Success Rate) directement ici,
        # donc visible round par round dans les logs Flower
...
if __name__ == "__main__":
    ...
    # ÉPINGLAGE CPU : on réserve le cœur CPU_CORE (=1) UNIQUEMENT pour ce client
    # psutil.Process(os.getpid()) : PSUTIL : récupère CE processus Python en cours
    # .cpu_affinity([CPU_CORE]) : PSUTIL : autorise CE processus uniquement sur ce cœur
    # Pourquoi : évite que ce client se batte avec le serveur ou les autres
    # clients pour les ressources CPU → mesures plus stables et reproductibles,
    # simulation plus proche de vrais Raspberry Pi séparés (chacun son propre CPU)
    ...
    # ArgumentParser() : classe PYTHON STANDARD (préinstallée)
    # permet de lire les arguments passés en ligne de commande
    ...
    # 3 choix possibles au lieu de 2 (bool) avant
    ...
    # on crée le client avec son identifiant FIGÉ (CLIENT_ID = 0) et son attaque
    client = MNISTClient(client_id=CLIENT_ID, attack_type=args.attack_type)

    # fl.client.start_client() : fonction FLOWER (préinstallée)
    # connecte ce client au serveur Flower et démarre la boucle FL
